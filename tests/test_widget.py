...
def test_invalid_number_of_card_account(invalid_len_number):
    with pytest.raises(ValueError):
        mask_account_card(invalid_len_number)


def test_empty_number_of_card_account(empty_number):
    with pytest.raises(ValueError):
        mask_account_card(empty_number)


def test_invalid_letter_number_of_card_account(letter_card_number):
    with pytest.raises(ValueError):
        mask_account_card(letter_card_number)


def test_invalid_number_only_one_word(letter_number):
    with pytest.raises(ValueError):
...
def test_invalid_len_get_date(invalid_len_date):
    with pytest.raises(ValueError):
...
def test_get_invalid_date(invalid_date, expected):
    with (pytest.raises(ValueError)):
