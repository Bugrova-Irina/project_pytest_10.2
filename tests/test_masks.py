...
def test_get_invalid_card_number(invalid_len_number):
    with pytest.raises(ValueError) as exc_info:
        get_mask_card_number(invalid_len_number)
    assert str(exc_info.value) == "Введен некорректный номер карты"


def test_get_invalid_letter_card_number(letter_number):
    with pytest.raises(ValueError) as exc_info:
        get_mask_card_number(letter_number)
    assert str(exc_info.value) == "Введен некорректный номер карты"


def test_get_invalid_empty_card_number(empty_number):
    with pytest.raises(ValueError) as exc_info:
...
def test_get_invalid_account_number(invalid_len_number):
    with pytest.raises(ValueError) as exc_info:
        get_mask_account(invalid_len_number)
    assert str(exc_info.value) == "Введен некорректный номер счета"


def test_get_invalid_letter_account(letter_number):
    with pytest.raises(ValueError) as exc_info:
        get_mask_account(letter_number)
    assert str(exc_info.value) == "Введен некорректный номер счета"


def test_get_invalid_empty_account(empty_number):
